...
while not version:
    version, ok = QInputDialog.getText(
        MainWindow,
        "Version Number",
        "What is the current version of the Nintendo Switch Online Mobile app?\nThe App Store says it is %s (Please enter like X.X.X)\nEnter nothing and press Okay to be sent to the app store's website."
        % version,
    )
    if not ok:
        quit()
...
"""
darkMode = (
    style.replace("#F2F2F2", "#2b2828")
    .replace("#dfdfdf", "#383333")
    .replace("#ffffff", "#e6e6e6")
    .replace("#fff", "#1e2024")
    .replace("#393939", "#c4bebe")
    .replace("#3c3c3c", "#fff")
)
...
class GUI(Ui_MainWindow):
    ...
    def setFriendCode(self, code):
        global settings
        settings["friendCode"] = code
        writeSettings()
        ...
        try:
            if platform.system() == "Windows":
                ...
                Shortcut = shell.CreateShortCut(
                    os.path.join(StartupFolder, "NSO-RPC.lnk")
                )
                ...
                if settings["startOnLaunch"] is False:
                    Shortcut.save()
                elif settings["startOnLaunch"] is True:
                ...
                if settings["startOnLaunch"] is False:
                    with open(
                        os.path.join(linuxServicePath, "NSO-RPC.service"), "w"
                    ) as out:
                        out.writelines(line + "\n" for line in linuxServiceFile)
                        out.close()
                    os.system(
                        "systemctl --user daemon-reload && systemctl --user enable NSO-RPC.service"
                    )
                else:
                ...
                applicationPath = os.path.join(
                    os.path.normpath(os.getcwd() + os.sep + os.pardir), "MacOS/NSO-RPC"
                )
                ...
                if settings["startOnLaunch"] is False:
                    with open(
                        os.path.join(macOSLaunchAgentPath, "NSO-RPC.plist"), "w"
                    ) as out:
                        out.writelines(line + "\n" for line in macOSplist)
                        out.close()
                else:
                    os.system(
                        "rm " + os.path.join(macOSLaunchAgentPath, "NSO-RPC.plist")
                    )
            else:
    ...
    def assignVariables(self):
        ...
        self.groupBox_2.findChild(QLabel, "label_5").setStyleSheet(
        ...
        [
            ...
            for x in (self.presenceText, self.presenceState)
        ]
        ...
        self.friendCode = QLineEdit(self.page_3, placeholderText="SW-1234-5678-9012")
        self.friendCode.setGeometry(QRect(101, 200, 200, 41))
        self.friendCode.setInputMask("SW-9999-9999-9999")
        self.friendCode.setText(settings.get("friendCode", ""))
        self.friendCode.textChanged.connect(self.setFriendCode)
    ...
    def grabToken(self):
        ...
        try:
            ...
            print(
                log(
                    f"An error occurred! Chances are, you didn't paste the right link, but here's the error message:\n{e}"
                )
            )
            os._exit(1)
    ...
    def updatePresence(self, user):
        ...
        try:
            text = "Friend Code: SW-%s" % str(
                user.links.get("friendCode").get("id")
            ).replace(" ", "-")
            state = ""
            ...
            self.label_21.setText(
                '<a href="https://github.com/MCMi460/NSO-RPC#quickstart-guide" style="color: cyan;">NSO-RPC Quickstart Guide</a>'
            )
        except:
            zone = "%Y/%m/%d"
            ...
            text = "When You Became Friends:\n%s" % time.strftime(
                zone, time.localtime(user.friendCreatedAt)
            )
        ...
        if not user.image:
            if user.nsaId not in iconsStorage:
        ...
        if user.presence.game.name:
            threading.Thread(
                target=up,
                args=(self.presenceImage, user.presence.game.imageUri),
                daemon=True,
            ).start()
    ...
    def checkDiscordError(self):
        ...
        if not client.rpc:
            ...
            if not ret[0]:
                ...
                elif "Could not find Discord" in str(
                    ret[1]
                ) or "Connection refused" in str(ret[1]):
    ...
    def updateProfile(self, new):
        ...
        dlg.setText(
            "You will need to restart the application in order for the changes to take place.\nSorry for the inconvenience."
        )
        sys.exit(dlg.exec_())
            ...
            try:
                for items in layout.itemAt(i).widget().findChildren(QGroupBox):
                    ...
                    for item in items.findChildren(QLabel):
                        if i == 1:
                            if client.api.friends[n].nsaId not in iconsStorage:
                                iconsStorage[client.api.friends[n].nsaId] = loadPix(
                                    client.api.friends[n].imageUri
                                )
                            client.api.friends[n].image = iconsStorage[
                                client.api.friends[n].nsaId
                            ]
    ...
    def updateFriends(self):
        ...
        for i in range(len(client.api.friends)):
            ...
            groupBox.setStyleSheet(
                ...
                % (
                    ("fff", "#dfdfdf")
                    if not settings["dark"]
                    else ("212020", "transparent")
                )
            )
        ...
        threading.Thread(
            target=self.setFriendIcons, args=(layout,), daemon=True
        ).start()
...
if __name__ == "__main__":
    ...
    if settings.get("startInSystemTray") is True:
        tray.show()
    else:
        tray.show()
        MainWindow.show()
